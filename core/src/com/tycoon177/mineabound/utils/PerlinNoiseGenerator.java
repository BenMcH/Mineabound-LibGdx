 ...
 * <p>
 *
 * The result is a continuous function that interpolates a smooth path along a series random points. The function is consistent, so given the same parameters, it will always return the same value. The smoothing function is based on the Improving Noise paper presented at Siggraph 2002.
 * <p>
...
public class PerlinNoiseGenerator {
	...
	public PerlinNoiseGenerator(int seed) {
		...
		while (--i > 0) {
			k = p_imp[i];
			j = (MathUtils.random(DEFAULT_SAMPLE_SIZE));
			p_imp[i] = p_imp[j];
			p_imp[j] = k;
		}
	...
	private void initPerlin1() {
		...
		for (i = 0; i < B; i++) {
			...
			g1[i] = (float) ((MathUtils.random(B + B)) - B) / B;

			for (j = 0; j < 2; j++)
				g2[i][j] = (float) ((MathUtils.random(B + B)) - B) / B;
			normalize2(g2[i]);

			for (j = 0; j < 3; j++)
				g3[i][j] = (float) ((MathUtils.random(B + B)) - B) / B;
			normalize3(g3[i]);
		}

		while (--i > 0) {
			k = p[i];
			j = (MathUtils.random(B));
			p[i] = p[j];
			p[j] = k;
		}
